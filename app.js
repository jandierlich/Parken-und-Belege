...
function renderSearchScreen() {
  const wrap = el('div', { style: 'display:flex;flex-direction:column;flex:1;min-height:0;overflow:hidden' });
  wrap.appendChild(header('Alles durchsuchen', 'Suche', { onBack: () => { state.screen = 'home'; render(); } }));
  const body = el('div', { class: 'content', style: 'padding-top:16px' });

  const searchInput = el('input', { type: 'text', placeholder: 'Ort, Titel, Notiz, Zweck …', id: 'global-search-input' });
