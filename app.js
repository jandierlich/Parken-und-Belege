...
function renderDashboard() {
  ...
  wrap.appendChild(header('Übersicht', 'Parken und Belege', {
    variant: 'dashboard',
  ...
  body.appendChild(el('div', { style: 'font-family:ui-monospace, monospace;font-size:10px;color:var(--sub);letter-spacing:1px;margin:14px 0 8px' }, 'IM ÜBERBLICK'));
  body.appendChild(el('div', { style: 'display:flex;gap:10px;margin-bottom:8px' }, [
    el('div', { class: 'card', style: 'flex:1;text-align:center;margin:0' }, [
  ...
  body.appendChild(el('div', { style: 'font-family:ui-monospace, monospace;font-size:10px;color:var(--sub);letter-spacing:1px;margin:16px 0 8px' }, 'BEREICHE'));

  const navItems = [
    { id: 'home', label: 'Parken', sub: 'Parkscheine erfassen', icon: mapPinSvg, color: '#5B2EE8' },
    { id: 'extra', label: 'Kosten', sub: 'Nebenkosten, Fahrten, km-Stand', icon: briefcaseSvg, color: '#00BFA6' },
    { id: 'map', label: 'Karte', sub: 'Alle Standorte im Überblick', icon: mapSvg, color: '#EC3D96' },
    { id: 'month', label: 'Monat & Jahr', sub: 'Übersicht, Export, Backup', icon: fileTextSvg, color: '#FF4757' },
    { id: 'notes', label: 'Notizen', sub: 'Freie Sprachaufnahmen', icon: micSvg, color: '#FF9500' },
  ];
  navItems.forEach(item => {
    body.appendChild(el('div', {
      class: 'card', style: 'display:flex;align-items:center;gap:14px;cursor:pointer;',
      onclick: () => { state.screen = item.id; render(); },
    }, [
      el('div', { style: `width:44px;height:44px;border-radius:16px;background:${item.color}1E;color:${item.color};display:flex;align-items:center;justify-content:center;flex-shrink:0` }, [icon(item.icon(), 20)]),
      el('div', { style: 'flex:1' }, [
        el('div', { style: 'font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px' }, item.label),
        el('div', { style: 'font-family:-apple-system, BlinkMacSystemFont, sans-serif;font-size:13px;color:var(--sub);margin-top:2px' }, item.sub),
      ]),
      el('span', { style: 'color:var(--sub);flex-shrink:0' }, [icon(chevronRightSvg(), 14)]),
    ]));
  });

  body.appendChild(el('div', { style: 'display:flex;gap:10px;margin-top:12px' }, [
    el('button', {
      style: 'flex:1;padding:12px;border-radius:22px;border:2px solid var(--line);background:var(--card);color:var(--violet);font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;',
...
function renderExtra() {
  ...
  const toggle = el('div', { style: 'display:flex;background:var(--card);border-radius:20px;padding:4px;margin-bottom:16px;box-shadow:0 4px 16px rgba(60,40,150,0.07)' }, [
    el('button', {
      style: `flex:1;padding:10px 3px;border:none;border-radius:16px;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;cursor:pointer;background:${state.kostenTab === 'nebenkosten' ? 'var(--violet)' : 'transparent'};color:${state.kostenTab === 'nebenkosten' ? '#fff' : 'var(--sub)'}`,
      onclick: () => { state.kostenTab = 'nebenkosten'; render(); },
    }, 'Nebenkosten'),
    el('button', {
      style: `flex:1;padding:10px 3px;border:none;border-radius:16px;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;cursor:pointer;background:${state.kostenTab === 'fahrten' ? 'var(--violet)' : 'transparent'};color:${state.kostenTab === 'fahrten' ? '#fff' : 'var(--sub)'}`,
      onclick: () => { state.kostenTab = 'fahrten'; render(); },
    }, 'Fahrten'),
    el('button', {
      style: `flex:1;padding:10px 3px;border:none;border-radius:16px;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;cursor:pointer;background:${state.kostenTab === 'kmstand' ? 'var(--violet)' : 'transparent'};color:${state.kostenTab === 'kmstand' ? '#fff' : 'var(--sub)'}`,
...
function renderAddExtra() {
  ...
  const dateWrap = el('div', { style: 'width:100%;overflow:hidden;border-radius:20px;border:2px solid var(--line);background:var(--card);margin-bottom:16px;padding:12px 14px;box-sizing:border-box;' }, [dateInput]);
  ...
  if (!existing || !existing.recurringSourceId) {
    const recurringRow = el('label', {
      style: 'display:flex;align-items:center;gap:10px;padding:12px 14px;border-radius:20px;border:2px solid var(--line);background:var(--card);margin-bottom:16px;cursor:pointer;',
    });
...
function renderMap() {
  ...
  if (lastKm) {
    ...
    infoCard.appendChild(el('div', { class: 'card', style: 'margin:0;border-left:4px solid var(--teal)' }, [
      ...
      el('a', {
        href: mapsUrl,
        style: 'display:flex;align-items:center;justify-content:center;gap:8px;padding:11px;border-radius:20px;background:linear-gradient(135deg,var(--teal),#008577);color:#fff;text-decoration:none;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px',
      }, [icon(mapPinSvg(), 15), ' Standort auf OpenStreetMap öffnen']),
    ]));
  }
...
function renderMonth() {
  ...
  const toggle = el('div', { style: 'display:flex;background:var(--card);border-radius:20px;padding:4px;margin-top:16px;margin-bottom:16px;box-shadow:0 4px 16px rgba(60,40,150,0.07)' }, [
    el('button', {
      style: `flex:1;padding:10px;border:none;border-radius:16px;font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;cursor:pointer;background:${state.overviewMode !== 'year' ? 'var(--violet)' : 'transparent'};color:${state.overviewMode !== 'year' ? '#fff' : 'var(--sub)'}`,
...
function showXLSXPreview(filtered) {
  ...
  function section(name, headers, rows) {
    ...
    h.style.cssText = "font-family:ui-rounded, -apple-system, BlinkMacSystemFont, sans-serif;font-weight:700;font-size:14px;color:#241F3D;margin-bottom:6px;";
    wrapDiv.appendChild(h);
    if (rows.length === 0) {
      const empty = document.createElement('div');
      empty.textContent = 'Keine Einträge in diesem Zeitraum.';
      empty.style.cssText = "font-family:-apple-system, BlinkMacSystemFont, sans-serif;font-size:13px;color:#8A85A6;";
      wrapDiv.appendChild(empty);
    } else {
      const table = document.createElement('table');
      table.style.cssText = 'width:100%;border-collapse:collapse;font-family:ui-monospace, monospace;font-size:11px;color:#241F3D;';
...
function showFilePreview(blob, mimeType) {
  ...
  if (mimeType === 'application/pdf') {
    ...
    box.innerHTML = `<div style="font-family:-apple-system, BlinkMacSystemFont, sans-serif;color:#241F3D;font-size:14px">Für diesen Dateityp gibt es keine Vorschau hier in der App.<br><br>Die Datei ist aber fertig und bereit zum Speichern.</div>`;
    overlay.appendChild(box);
  } else {
    ...
    pre.style.cssText = "margin:0;white-space:pre-wrap;word-break:break-word;font-family:ui-monospace, monospace;font-size:13px;color:#241F3D;";
