...
function renderAddExtra() {
  ...
  const titelInput = el('input', { type: 'text', value: existing?.titel || '', placeholder: 'z. B. Tankstelle A5' });
  body.appendChild(titelInput);

  body.appendChild(el('label', { class: 'field-label' }, 'Kategorie'));
  const catSelect = el('select', {}, state.categories.map(c => el('option', { value: c, selected: (existing?.kategorie === c) ? 'selected' : null }, c)));
  ...
  const betragInput = el('input', { type: 'text', inputmode: 'decimal', value: existing?.betrag ?? '', placeholder: '0,00' });
  body.appendChild(betragInput);

  body.appendChild(el('label', { class: 'field-label' }, 'Notiz (optional)'));
  const notizInput = el('textarea', {}, existing?.notiz || '');
...
function renderAddFahrt() {
  ...
  const vonInput = el('input', { type: 'text', value: existing?.von || '', placeholder: 'z. B. Büro' });
  body.appendChild(vonInput);

  body.appendChild(el('label', { class: 'field-label' }, 'Nach'));
  const nachInput = el('input', { type: 'text', value: existing?.nach || '', placeholder: 'z. B. Kunde Meier GmbH' });
  body.appendChild(nachInput);

  body.appendChild(el('label', { class: 'field-label' }, 'Zweck (optional)'));
  const zweckInput = el('input', { type: 'text', value: existing?.zweck || '', placeholder: 'z. B. Kundentermin' });
  body.appendChild(zweckInput);

  body.appendChild(el('label', { class: 'field-label' }, 'Gefahrene Kilometer'));
  const kmInput = el('input', { type: 'text', inputmode: 'decimal', value: existing?.km ?? '', placeholder: '0' });
